...
#### 3. [Bellman-Ford](https://www.geeksforgeeks.org/bellman-ford-algorithm-simple-implementation/)
注意到 dijkstra 中找最短邊的判斷為```if (dist[y] > dist[u] + w)```，所以當權重為負數時就會沒辦法跳脫 while 循環陷入無窮迴圈，因為會一直把同一條邊放入 queue 中。Bellman-Ford 則使用對每個邊做判斷改避免這問題
```cpp
void BellmanFord(int graph[][3], int V, int E, int src) {
    // Initialize distance of all vertices as infinite.
    // (u, v, w) from vertex u to v. And weight is w.
    int dis[V] = {INT_MAX};
    dis[src] = 0;
